    ...
    return (
        <div className="flex flex-col gap-6 pt-2">
            ...
            <div className="space-y-4">
                ...
                {partsCount > 1 && (
                    <div className="space-y-2 pt-2 border-t border-border/50">
                        ...
                        <SliderControl
                            param={{ id: 'explode_factor', type: 'slider', min: 0, max: 2, step: 0.1, default: 0, label: { en: t('ctrl.explode_factor') } }}
                            value={explodeFactor}
                            onSliderChange={(_: string, val: number[]) => setExplodeFactor(val[0])}
                            getLabel={getLabel}
                            language={language}
                            t={t}
                        />
                ...
                <div className="space-y-2">
                    ...
                    {clippingEnabled && (
                        <div className="space-y-2 pl-2">
                            ...
                            <SliderControl
                                param={{ id: 'clipping_position', type: 'slider', min: 0, max: 1, step: 0.01, default: 0.5, label: { en: t('ctrl.clipping_position') } }}
                                value={clippingPosition}
                                onSliderChange={(_: string, val: number[]) => setClippingPosition(val[0])}
                                getLabel={getLabel}
                                language={language}
                                t={t}
                            />
                ...
                {setOverhangEnabled && (
                    <div className="space-y-2 pt-2 border-t border-border/50">
                        ...
                        {overhangEnabled && setOverhangThreshold && (
                            <div className="pl-2">
                                <SliderControl
                                    param={{ id: 'overhang_threshold', type: 'slider', min: 20, max: 80, step: 1, default: 45, label: { en: t('ctrl.overhang_threshold') } }}
                                    value={overhangThreshold}
                                    onSliderChange={(_: string, val: number[]) => setOverhangThreshold(val[0])}
                                    getLabel={getLabel}
                                    language={language}
                                    t={t}
                                />
            ...
            <div className="space-y-2 border-t border-border pt-4">
                ...
                <SliderControl
                    param={{ id: 'light_intensity', type: 'slider', min: 0.1, max: 2, step: 0.1, default: 1, label: { en: t('ctrl.brightness') } }}
                    value={lightIntensity}
                    onSliderChange={(_: string, val: number[]) => setLightIntensity(val[0])}
                    getLabel={getLabel}
                    language={language}
                    t={t}
                />
                <div className="space-y-1">
                    ...
                    <select
                        ...
                        onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setEnvironmentPreset(e.target.value)}
                    >
                    ...
                    <div className={`grid gap-2 ${partColors.length > 1 ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1'}`}>
                        {partColors.map((part) => (
                            <div key={part.id} className="flex flex-col gap-1">
                                <Label htmlFor={`color-${part.id}`} className="text-xs">{getLabel(part as unknown as Record<string, unknown>, 'label', language)}</Label>
                                <input
                                    id={`color-${part.id}`}
                                    type="color"
                                    className="w-full h-11 min-h-[44px] cursor-pointer"
                                    value={colors[part.id] || part.default_color}
                                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleColorChange(part.id, e.target.value)}
                                />
                            </div>
                        ))}
                    </div>
