...
describe('RenderQueuePanel', () => {
    beforeEach(() => {
        ...
        global.fetch = mockFetchResolving(BUSY)
    })
    afterEach(() => {
        vi.restoreAllMocks()
    })
    ...
    it('queries the active renders endpoint', async () => {
        ...
        expect(global.fetch).toHaveBeenCalledWith('/api/admin/renders/active', expect.any(Object))
    })
    ...
    it('sends the bearer token when one is stored', async () => {
        ...
        render(<RenderQueuePanel />)
        await screen.findByText('2 active')
        expect(global.fetch).toHaveBeenCalledWith('/api/admin/renders/active', expect.objectContaining({
    ...
    it('falls back to the secondary badge and empty state when idle', async () => {
        global.fetch = mockFetchResolving({ active_renders: 0, recent: [] })
        render(<RenderQueuePanel />)
    ...
    it('shows the empty state when recent is absent entirely', async () => {
        global.fetch = mockFetchResolving({ active_renders: 0 })
        render(<RenderQueuePanel />)
    ...
    it('omits the note block when the payload has none', async () => {
        global.fetch = mockFetchResolving({ active_renders: 1, recent: [] })
        render(<RenderQueuePanel />)
    ...
    it('surfaces an HTTP error and retries on demand', async () => {
        global.fetch = mockFetchResolving(null, false, 500)
        render(<RenderQueuePanel />)
    ...
    it('surfaces a network rejection', async () => {
        global.fetch = vi.fn(() => Promise.reject(new Error('connection reset')))
        render(<RenderQueuePanel />)
