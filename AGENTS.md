...
- **Studio E2E**: Playwright -- 23 test suites in `apps/studio/e2e/tests/`, page object pattern, mock API via `api-mocker.js`. Suite `23-browser-audit/` runs against a real backend with OpenSCAD (no API mocks) -- use `--project=audit`; the nightly `e2e-audit.yml` provisions that backend on the runner itself, without Docker (#76)
...
| E2E test patterns | Use Playwright's `toHaveText`/`toBeEnabled` assertions instead of `waitForTimeout` + `textContent()`. Auto-render caches results -- change a param to bust cache before testing slow/error mocks. `editSliderValue` commits via Enter key. Native `<input type="color">` cannot be programmatically set in Playwright. Landing E2E tests in `12-responsive/` require `LANDING_URL` env var (skipped otherwise). Browser audit tests in `23-browser-audit/` need a **real backend on :5000 with OpenSCAD** -- run via `npx playwright test --project=audit`; `skipIfNoBackend()` skips them when `/api/health` is unreachable or reports `openscad` not ok, whatever is serving it. Docker Compose is one way to provide that locally, but it is **not** how the nightly runs: since #76 (2026-09-02) `e2e-audit.yml` starts the backend, Redis and the render worker directly on the runner, the same shape as `ci.yml`'s e2e job -- the Compose plugin is absent from `madfam-runners-blue`, which had silently skipped the audit's test step on every run since 2026-03-21. Default browser projects (chromium/firefox/webkit) exclude the audit suite via `testIgnore` |
