...
# diameter, so the snap-slot cut lands tangent to the bore wall. OCCT keeps that
# as one valid shell, but the tangency tessellates into a cracked (non-watertight)
# STL. Floor the clearance at a hair above zero: geometrically a press fit still,
# kernel-wise a real intersection.
clearance = max(0.02, min(clearance, 1.0))
...
def build_strain_comb():
    ...
    body = body.cut(tie)
    # Two mounting holes through the bar ends. The centre inset must leave real
    # material between the bore wall and the (filleted) end face: at wall=2.0 the
    # old inset of `wall` put the 2.0 mm bore edge exactly ON bar_l/2, tangent to
    # the end face AND grazing the corner fillet — a tangency the STL tessellates
    # as a torn shell. Inset by the hole radius plus the fillet plus a real ligament.
    hole_r = 2.0
    fillet_r = min(3.0, bar_w / 4.0)
    inset = max(wall, hole_r + fillet_r * 0.5 + 0.8)
    for sx in (-1, 1):
        hole = (
            cq.Workplane("XY").transformed(offset=cq.Vector(sx * (bar_l / 2.0 - inset), 0, -0.5))
            .circle(hole_r).extrude(bar_h + 1.0)
        )
        body = body.cut(hole)
    return body
