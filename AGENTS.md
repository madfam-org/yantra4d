...
| Render placement | The **browser is the default**: rendering there is free for us and unmetered for the visitor. `decideRenderPlacement()` (`services/engine/renderPlacement.ts`) is a PURE function with a 10-rule precedence table -- hard rules first (the MODE's engine is `cadquery`/`graph`/`implicit`, manifest `render.server_only`, a bundle with `unsupported` or `unresolved` entries), then `?render=`/`VITE_RENDER_MODE`, the visitor's `render_placement_preference`, capability tier, this session's browser failures for the slug, the estimate threshold (capable 45 s / limited 15 s), the SOFT `force_backend` hint (only on a `limited` device), and finally **browser**. The old `if (API_BASE) return 'backend'` server pin is GONE: production always sets `VITE_API_BASE`, so every heuristic below it was dead code and every render was billed to us and metered to the visitor. The sticky module-global `_hardwareMode` is gone too -- placement is keyed per slug. Engine is resolved PER MODE by `effectiveModeEngine()`, mirroring `ProjectManifest.mode_engine()`: 8 of the 31 OpenSCAD-capable cartridges are dual-engine (gridfinity is `project.engine: cadquery` with 3 explicit `openscad` modes), and reading the project engine alone would keep every one of those modes on the metered path |
