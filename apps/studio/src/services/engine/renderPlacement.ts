...
export function decideRenderPlacement(input: PlacementInput): PlacementDecision {
  const {
    ...
    browserMaxEstimateSeconds = null,
  ...
  const threshold = (
    browserMaxEstimateSeconds !== null
    && browserMaxEstimateSeconds !== undefined
    && Number.isFinite(browserMaxEstimateSeconds)
    && browserMaxEstimateSeconds >= 0
  )
    ? browserMaxEstimateSeconds
    : ESTIMATE_THRESHOLD_SECONDS[capabilityTier]
  if (
    estimateSeconds !== null
