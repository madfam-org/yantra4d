...
async function buildGlb(specs: MeshSpec[]): Promise<Uint8Array> {
  ...
  for (const spec of specs) {
    const position = doc.createAccessor().setType('VEC3').setArray(spec.positions).setBuffer(buffer)
    const indices = doc.createAccessor().setType('SCALAR').setArray(spec.indices).setBuffer(buffer)
    const prim = doc
      .createPrimitive()
    ...
    if (spec.material) {
      const normals = doc
        .createAccessor()
        .setType('VEC3')
        .setArray(new Float32Array(spec.positions.length).fill(0).map((_, i) => (i % 3 === 1 ? 1 : 0)))
        .setBuffer(buffer)
      prim.setAttribute('NORMAL', normals)
      prim.setMaterial(doc.createMaterial('paint').setBaseColorFactor([1, 0, 0, 1]))
    }
...
function makeRepo(spec: RepoSpec = {}): string {
  ...
  if (spec.raw) {
    ...
    for (const [name, bytes] of Object.entries(spec.raw)) fs.writeFileSync(path.join(models, 'raw', name), bytes)
  }
  for (const [name, bytes] of Object.entries(spec.legacy ?? {})) fs.writeFileSync(path.join(models, name), bytes)
  for (const m of spec.manifests ?? []) {
    const dir = path.join(repo, 'projects', m.slug)
    fs.mkdirSync(dir, { recursive: true })
    const manifest: Record<string, unknown> = { project: { slug: m.slug, name: m.slug } }
    if (m.animations) {
      manifest.animations = [{ id: 'sweep', label: 'Sweep', from_state: { a: 1 }, to_state: { a: 2 }, frames: 2 }]
    }
    fs.writeFileSync(path.join(dir, 'project.json'), JSON.stringify(manifest))
  }
  if (spec.hero) fs.writeFileSync(path.join(landing, 'models.hero.json'), JSON.stringify(spec.hero))
  return repo
}

const modelsDir = (repo: string) => path.join(repo, 'apps', 'landing', 'public', 'models')
const readManifest = (repo: string) => JSON.parse(fs.readFileSync(path.join(modelsDir(repo), 'manifest.json'), 'utf8'))
...
async function optimize(repo: string, argv: string[] = [], extra: Record<string, unknown> = {}) {
  const io = capture()
  const code = await run({
    argv: ['--generated', '2026-09-19T00:00:00Z', '--commons-pin', 'none', ...argv],
    repo,
    env: {},
    ...io,
    ...extra,
...
async function triangleCountOf(bytes: Uint8Array) {
  ...
  for (const mesh of doc.getRoot().listMeshes()) {
    for (const prim of mesh.listPrimitives()) count += prim.getIndices()!.getCount() / 3
  }
  return { doc, count }
}
...
describe('parseInputName', () => {
  it('recognises base renders and keyframes', () => {
    expect(parseInputName('gridfinity.glb')).toEqual({ kind: 'base', slug: 'gridfinity' })
...
describe('parseArgs', () => {
  it('accepts both flag spellings and rejects the rest', () => {
    expect(parseArgs(['--lod0', 'hero', '--strict', '--generated=2026-01-01T00:00:00Z'])).toEqual({
      lod0: 'hero',
      strict: true,
...
describe('optimizeGlb', () => {
  it('simplifies a dense mesh under the triangle budget, meshopt-compressed and POSITION-only', async () => {
    ...
    expect(Object.keys(json.meshes[0].primitives[0].attributes)).toEqual(['POSITION'])
  ...
  it('bakes every node into one world-space primitive, dropping materials and normals', async () => {
    ...
    const { before, outputs } = await optimizeGlb(glb, { lod1: { triangles: 1000, bytes: 100_000 } }, deps)
  ...
  it('lowers the triangle target when the byte budget is the binding one', async () => {
    const { outputs } = await optimizeGlb(dense, { lod0: { triangles: 4000, bytes: 6000 } }, deps)
    const lod0 = outputs.lod0!
...
describe('run — raw inputs', () => {
  it('writes lod files, keyframes and the v2 manifest', async () => {
    const repo = makeRepo({
      raw: { 'gridfinity.glb': dense, 'motor-mount.glb': small, 'motor-mount.sweep.1.glb': dense, 'motor-mount.sweep.0.glb': small },
    })
    const { code, err } = await optimize(repo, ['--commons-pin', 'f2c578f78037c37f0f29fe0571b946aeeb45bc0f'])
    expect(err).toEqual([])
    ...
    expect(Object.keys(manifest)).toEqual(['version', 'generated', 'generator', 'source', 'budgets', 'models'])
    expect(manifest.version).toBe(MANIFEST_VERSION)
    expect(manifest.generated).toBe('2026-09-19T00:00:00Z')
    expect(manifest.generator).toBe(GENERATOR)
    expect(manifest.source).toEqual({ kind: 'render-api', commons_pin: 'f2c578f78037c37f0f29fe0571b946aeeb45bc0f' })
    expect(manifest.budgets).toEqual(manifestBudgets(BUDGETS))
    expect(manifest.budgets._comment).toBeUndefined()

    expect(manifest.models.map((m: { slug: string }) => m.slug)).toEqual(['gridfinity', 'motor-mount'])
    const [gridfinity, motorMount] = manifest.models
    expect(Object.keys(gridfinity)).toEqual(['slug', 'size', 'lod1', 'lod0'])
    expect(Object.keys(gridfinity.lod1)).toEqual(['file', 'bytes', 'triangles'])
    expect(gridfinity.lod1.file).toBe('gridfinity.lod1.glb')
    expect(gridfinity.lod1.bytes).toBe(fs.statSync(path.join(modelsDir(repo), 'gridfinity.lod1.glb')).size)
    expect(gridfinity.lod1.triangles).toBeLessThanOrEqual(BUDGETS.lod1Triangles)
    expect(gridfinity.lod0.triangles).toBeGreaterThan(gridfinity.lod1.triangles)
    // v1 readers keep working on slug + size: size is the smallest file the entry offers.
    expect(gridfinity.size).toBe(Math.min(gridfinity.lod1.bytes, gridfinity.lod0.bytes))
  ...
  it('is byte-identical across two runs with the same --generated', async () => {
    const a = makeRepo({ raw: { 'gridfinity.glb': dense, 'motor-mount.sweep.0.glb': small } })
    const b = makeRepo({ raw: { 'gridfinity.glb': dense, 'motor-mount.sweep.0.glb': small } })
    expect((await optimize(a)).code).toBe(EXIT_OK)
    expect((await optimize(b)).code).toBe(EXIT_OK)
    const names = fs.readdirSync(modelsDir(a)).filter((n) => n !== 'raw').sort()
    expect(names).toEqual(fs.readdirSync(modelsDir(b)).filter((n) => n !== 'raw').sort())
    expect(names.length).toBeGreaterThan(2)
    for (const name of names) {
      expect(fs.readFileSync(path.join(modelsDir(a), name)).equals(fs.readFileSync(path.join(modelsDir(b), name)))).toBe(true)
    }
  })
  ...
  it('exits 1 under --strict when a file cannot meet its byte budget, and still writes it', async () => {
    ...
    expect(relaxed.err.join('\n')).toMatch(/WARNING: 1 output\(s\) over their byte budget/)
  ...
  it('appends the report to $GITHUB_STEP_SUMMARY when set', async () => {
    ...
    const io = capture()
    const code = await run({ argv: ['--generated', '2026-09-19T00:00:00Z', '--commons-pin', 'none'], repo, env: { GITHUB_STEP_SUMMARY: summary }, ...io })
    expect(code).toBe(EXIT_OK)
  ...
  it('reports an unreadable input as a failure without dropping the others', async () => {
    const repo = makeRepo({ raw: { 'broken.glb': new Uint8Array([1, 2, 3, 4]), 'gridfinity.glb': small } })
    const { code, err } = await optimize(repo)
    expect(code).toBe(EXIT_FAILED)
    expect(err.join('\n')).toMatch(/FAILED broken\.glb/)
    expect(fs.existsSync(path.join(modelsDir(repo), 'gridfinity.lod1.glb'))).toBe(true)
    expect(readManifest(repo).models.map((m: { slug: string }) => m.slug)).toEqual(['gridfinity'])
  })
...
describe('run --check / --clean', () => {
  ...
  it('reuses the on-disk manifest timestamp under --check, so only content counts as drift', async () => {
    ...
    expect(await run({ argv: ['--check', '--commons-pin', 'none'], repo, env: {}, ...io })).toBe(EXIT_OK)
  })
...
describe('run — legacy uncompressed inputs', () => {
  it('reads public/models/<slug>.glb when there is no raw dir and removes it only with --clean', async () => {
    const repo = makeRepo({ legacy: { 'gridfinity.glb': dense, 'motor-mount.glb': small } })
    ...
    expect(first.err.join('\n')).toMatch(/would be removed by --clean: gridfinity\.glb, motor-mount\.glb/)

    const second = await optimize(repo, ['--clean'])
    expect(second.code).toBe(EXIT_OK)
    expect(fs.existsSync(legacy)).toBe(false)
    expect(fs.existsSync(path.join(modelsDir(repo), 'motor-mount.glb'))).toBe(false)
    ...
    expect(readManifest(repo).models.map((m: { slug: string }) => m.slug)).toEqual(['gridfinity', 'motor-mount'])
  ...
  it('never treats its own outputs as inputs', async () => {
    ...
    expect(fs.readdirSync(modelsDir(repo)).filter((n) => n.includes('.lod1.lod'))).toEqual([])
  })
})
...
describe('resolveLod0Selection', () => {
  it('defaults to all for a small set and to hero above the threshold', () => {
    const repo = makeRepo({ manifests: [{ slug: 'motor-mount', animations: true }, { slug: 'gridfinity' }] })
    const ctx = makeContext({ repo })
    const few = ['gridfinity', 'motor-mount']
    expect(resolveLod0Selection(undefined, few, ctx)).toEqual({ mode: 'all', slugs: new Set(few) })

    const many = Array.from({ length: LOD0_ALL_MAX_INPUTS + 1 }, (_, i) => `cartridge-${i}`).concat(few)
  ...
  it('prefers models.hero.json over the animated manifests', () => {
    const repo = makeRepo({ manifests: [{ slug: 'motor-mount', animations: true }], hero: ['gridfinity'] })
    const ctx = makeContext({ repo })
    expect([...resolveLod0Selection('hero', ['gridfinity', 'motor-mount'], ctx).slugs]).toEqual(['gridfinity'])
  })

  it('accepts an explicit list, ignoring slugs with no input, and none', () => {
    const ctx = makeContext({ repo: makeRepo() })
    const list = resolveLod0Selection('gridfinity, nope', ['gridfinity', 'motor-mount'], ctx)
    expect([...list.slugs]).toEqual(['gridfinity'])
    expect(list.unknown).toEqual(['nope'])
    expect(resolveLod0Selection('none', ['gridfinity'], ctx).slugs.size).toBe(0)
  })

  it('--lod0 hero from the CLI emits lod0 for animated cartridges only', async () => {
    const repo = makeRepo({
      raw: { 'gridfinity.glb': dense, 'motor-mount.glb': dense },
      manifests: [{ slug: 'motor-mount', animations: true }, { slug: 'gridfinity' }],
    })
...
describe('buildManifest', () => {
  it('sorts models by slug and frames by animation then index', () => {
    const entries = new Map([
      ['zeta', { lod1: { file: 'zeta.lod1.glb', bytes: 10, triangles: 5 }, lod0: null, frames: [] }],
      [
        'alpha',
        {
          ...
          frames: [
            { file: 'alpha.b.1.glb', bytes: 8, triangles: 1, animation: 'b', index: 1 },
            { file: 'alpha.a.10.glb', bytes: 9, triangles: 1, animation: 'a', index: 10 },
            { file: 'alpha.a.2.glb', bytes: 7, triangles: 1, animation: 'a', index: 2 },
          ],
        },
      ],
    ])
    const manifest = buildManifest({ generated: 'g', sourceKind: 'render-api', commonsPin: null, budgets: BUDGETS, entries })
    expect(manifest.models.map((m: { slug: string }) => m.slug)).toEqual(['alpha', 'zeta'])
