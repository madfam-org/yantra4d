...
def build_chair_crossbar():
    ...
    # The re-bore MUST NOT be exactly `_clip_ir` (= bar_dia/2 + 0.4). At that radius
    # the cutter's wall lands coincident with the saddle's own bore wall, and cutting
    # a face onto itself is the classic zero-thickness boolean: OCCT returns a shape
    # whose STL tessellates as a torn shell (watertight=False, bodies=0). Shaving a
    # hundredth of a millimetre off makes the cutter strictly INSIDE the saddle bore,
    # so it only ever removes column material and leaves the saddle wall untouched.
    bore_r = _clip_ir - 0.01
