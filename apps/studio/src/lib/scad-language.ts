...
interface MonacoLanguages {
  ...
  register(languageDef: { id: string; extensions?: string[] }): void
  setMonarchTokensProvider(languageId: string, provider: MonarchLanguageProvider): void
  registerCompletionItemProvider(languageId: string, provider: CompletionItemProvider): void
  CompletionItemKind: {
    Keyword: number
  }
}

interface MonarchLanguageProvider {
  keywords: string[]
  builtins: string[]
  operators: string[]
  symbols: RegExp
  tokenizer: Record<string, Array<[RegExp, string | { cases: Record<string, string> }, string?]>>
}
...
interface MonacoCompletionModel {
  getWordUntilPosition(position: MonacoCompletionPosition): { startColumn: number; endColumn: number }
}

interface MonacoCompletionPosition {
  lineNumber: number
  column: number
}
...
export function registerScadLanguage(monaco: MonacoInstance): void {
