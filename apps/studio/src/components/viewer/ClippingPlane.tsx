...
import { Vector3, Plane, DoubleSide, Box3 } from 'three'
...
export default function ClippingPlane({ axis = 'z', position = 0.5, bbox }: ClippingPlaneProps) {
  const { gl } = useThree()
  ...
  const planeSize = useMemo(() => {
    if (!bbox) return [200, 200] as [number, number]
