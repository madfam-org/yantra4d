...
| Touch targets | All interactive elements enforce WCAG 2.5.8 minimum 44px touch targets on mobile via `min-h-[44px] min-w-[44px]`. Inputs use `text-base` on mobile to prevent iOS Safari auto-zoom (global rule in `index.css` forces 16px on `select/input/textarea` below 768px). AI chat input uses `text-base sm:text-xs` to prevent iOS zoom. SynthesisModal textarea uses `text-base sm:text-sm`. Slider numeric inputs have `inputMode="decimal"` for mobile number pad. 3D viewer container has `touch-action: none` to prevent browser gesture conflicts with Three.js OrbitControls. Focus-visible rings are 3px/4px offset on coarse pointer devices. Onboarding wizard inputs (EditStep, UploadStep) enforce 44px touch targets and mobile-friendly padding. PrintEstimateOverlay selects use `py-2 sm:py-0.5` + `min-h-[44px] sm:min-h-0`. ComponentPicker and grid preset buttons use `min-h-[44px] sm:min-h-0`. TpmsTopologyControl buttons enforce `min-h-[44px]`. EnergySliderControl range thumb enlarged to 28px via webkit/moz pseudo-element selectors. AuthButton OAuth dropdown items use `py-3 sm:py-1.5` + `min-h-[44px] sm:min-h-0`. GitPanel file checkboxes and diff summary use `py-2 sm:py-0.5` + `min-h-[44px] sm:min-h-0`. Landing CTA buttons use `min-h-[44px] inline-flex items-center justify-center` |
