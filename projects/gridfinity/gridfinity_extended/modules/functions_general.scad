...
function sum(list, c = 0, end) =
  let (end = is_undef(end) ? len(list) : end) c < 0 || end < 0 ? 0
  : c < len(list) - 1 && c < end ? list[c] + sum(list, c + 1, end=end)
  : list[c];

function vector_sum(v, start = 0, end, itemIndex) =
  let (v = is_list(v) ? v : [v], end = is_undef(end) ? len(v) - 1 : min(len(v) - 1, end)) is_num(itemIndex) ? start < end ? v[start][itemIndex] + vector_sum(v, start + 1, end, itemIndex) : v[start][itemIndex]
...
function DictSetRange(list, keyValueArray) =
  !(len(keyValueArray) > 0) ? list
  ...
  let (currentKeyValue = keyValueArray[0]) assert(is_list(currentKeyValue), str("DictSetRange(keyValueArray, arr) - currentKeyValue is not a list. currentKeyValue:", currentKeyValue))
...
function DictSet(list, keyValue) =
  ...
  let (
    matchResults = search([keyValue[0]], list, 1),
    matchIndex = is_list(matchResults) && len(matchResults) == 1 && is_num(matchResults[0]) ? matchResults[0] : undef
...
function DictToString(list, name = "") =
  let (infoText = [for (i = [0:len(list) - 1]) str(list[i][0], "=", list[i][1])]) str("🟧", name, concatstringarray(infoText));
...
function replace_Items(keyValueArray, arr) =
  !(len(keyValueArray) > 0) ? arr
  ...
  let (currentKeyValue = keyValueArray[0]) assert(is_list(currentKeyValue), "replace_Items(keyValueArray, arr) - currentKeyValue is not a list")
...
function splitCustomConfig(customConfig) =
  let (
...
module assert_openscad_version() {
  assert(version()[0] > 2020, "Gridfinity Extended requires OpenSCAD version 2021.01 or later. https://openscad.org/downloads.");
}
