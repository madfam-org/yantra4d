...
- **Why it is still not a CI lane**: measured on 2026-09-02 over the full commons (the 35 public cartridge submodules plus `libs/`, matching `test-geometric-parity`'s recursive checkout), the audit takes **5m38s** and reports **10 of 29 comparable pairs passing** — 17 pairs diverge geometrically between the two kernels, one cartridge is refused by the CadQuery sandbox, one fails to render in OpenSCAD. The remaining 1417 modes have no comparable pair at all (1397 CadQuery-only placeholders, 20 OpenSCAD-only). So the Parity Guarantee below describes an intent the commons does not currently meet; wiring the tool into CI would fail the build on day one. Closing those 19 gaps is the prerequisite, not a change to the tool.
