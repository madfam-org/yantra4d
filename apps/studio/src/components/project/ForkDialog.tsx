...
export default function ForkDialog({ slug, projectName, onClose, onForked }: ForkDialogProps) {
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div role="dialog" aria-modal="true" aria-label="Fork project" className="bg-card rounded-lg border border-border shadow-lg p-6 max-w-[95vw] sm:max-w-md w-full mx-4 max-h-[90dvh] overflow-y-auto" onClick={(e: React.MouseEvent) => e.stopPropagation()} onKeyDown={(e: React.KeyboardEvent) => { if (e.key === 'Escape') onClose() }}>
        ...
        <input
          ...
          type="text"
          value={newSlug}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewSlug(sanitizeSlug(e.target.value))}
