...
**Effort**: Small — 1–2 hours of CI config work.

---

#### P0.2 — Resolve Stub Projects
...
- All other previously-stub projects (`din-rail-clip`, `faircap-filter`, `glia-diagnostic`, `parametric-connector`, `prosthetic-socket`, `soft-jaw`) already had `project.json` and SCAD files — they only needed `estimate_constants` and schema fixes.
- All 28 active projects now pass `scripts/validate_manifests.py`. ✨
...
#### P1.1 — Complete Project Gallery

**Status**: ✅ **Complete**

All 36 active projects now have `thumbnail`, `tags`, and `difficulty` fields. `glia-diagnostic` was one of the last projects missing these — added as part of the P0.2 schema cleanup pass.

---

#### P1.2 — Per-Project Documentation

**Status**: ✅ **Complete**

All 36 active projects now have `docs/README.md`, auto-generated by `scripts/generate-project-docs.py`. `framing-hyperobject` has a hand-written `docs/README.md` describing the research document and planned CDG interfaces.

---

#### P1.3 — WASM Fallback Testing
...
#### P1.4 — Rate Limiter Backend (Redis)
...
#### P1.5 — Hyperobjects Commons Phase 2 (UI)
...
**Risk**: Low — BOSL2 is already a submodule in `libs/`. The `gear-reducer` project already demonstrates the BOSL2 gear API.
...
**Concept**: The totality of framing systems — from gilded museum mouldings to sonic-welded trading card slabs, from SEG lightbox extrusions to VESA monitor mounts — constitutes a massive, distributed infrastructure of containment. This hyperobject is governed by rigorous, often invisible, standards of metrology, material physics, and industrial tolerance.

**Research covers** (10 sections, 70+ citations):
- Traditional picture frames: rabbet geometry, moulding profiles (bevel, scoop, ovolo, ogee/cyma, floater), archival hierarchy (museum → conservation → budget)
...
- Industrial snap frames: spring-steel mechanism, 25mm/32mm profile standards, visual bite (10–12mm)
- LED lightboxes: edge-lit vs. ladder-light, depth vs. diffusion tradeoffs (18mm–160mm)
...
- Wearable frames (ID badges): CR80/ISO 7810 standard, slot punch geometry (1/8" × 9/16"), lanyard hardware
- Parametric digital frames: OpenSCAD + BOSL2 `path_sweep()` / `bezier_sweep()` for ogee profiles, FDM friction-fit tolerances (0.15–0.25mm clearance)
...
|------|----------|--------|--------|--------|
| E2E tests in CI | **P0** | High | Small | ✅ Done |
| Resolve stub projects | **P0** | Medium | Small | ✅ Done |
| Complete project gallery | **P1** | High | Medium | ✅ Done |
| Per-project documentation | **P1** | Medium | Medium | ✅ Done |
