...
interface SliderControlProps {
    ...
    onSliderChange: (name: string, valArray: number[], commit?: boolean) => void
...
export default function SliderControl({ param, value, onSliderChange, getLabel, language, t, unitSystem }: SliderControlProps) {
    ...
    const rawLabel = getLabel(param as unknown as Record<string, unknown>, 'label', 'en') || ''
    ...
    const paramLabel = isInches
        ? getLabel(param as unknown as Record<string, unknown>, 'label', language).replace('(mm)', '(in)')
    ...
    return (
        <div className={`space-y-1 ${isDisabled ? 'opacity-50 pointer-events-none' : ''}`}>
                ...
                <div className="flex-shrink-0 text-right w-14">
                    {editing ? (
                        <input
                            ...
                            onBlur={commitEdit}
                            onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => { if (e.key === 'Enter') commitEdit(); if (e.key === 'Escape') setEditing(false) }}
                        />
                    ) : (
                        <span
                            ...
                            onKeyDown={(e: React.KeyboardEvent<HTMLSpanElement>) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setEditing(true); setEditValue(String(displayValue)) } }}
                        >
                            {displayValue}
                        </span>
                    )}
            ...
            <Slider
                value={[safeValue]}
                min={param.min} max={param.max} step={param.step}
                onValueChange={(vals: number[]) => onSliderChange(param.id, vals, false)}
                onValueCommit={(vals: number[]) => onSliderChange(param.id, vals, true)}
            ...
            {param.description && (
                <p className="text-xs text-muted-foreground">{getLabel(param as unknown as Record<string, unknown>, 'description', language)}</p>
            )}
        </div>
    )
}
