...
interface EnergySliderControlProps {
    value: number | undefined
    onChange: (name: string, value: number) => void
    thermodynamics: Thermodynamics | null
    language: string
}

export default function EnergySliderControl({ value, onChange, thermodynamics, language }: EnergySliderControlProps) {
        ...
        <div className="flex flex-col space-y-2 mt-4 p-3 border border-border rounded bg-muted/20">
            ...
            <div className="flex items-center space-x-3">
                ...
                <input
                    ...
                    step={1}
                    value={value || 0}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChange('simulated_energy', parseFloat(e.target.value))}
                    className="flex-1 h-3 md:h-2 min-h-[44px] bg-secondary rounded-lg appearance-none cursor-pointer accent-primary [&::-webkit-slider-thumb]:h-11 [&::-webkit-slider-thumb]:w-11 [&::-moz-range-thumb]:h-11 [&::-moz-range-thumb]:w-11"
                />
