...
All togglable parameters (`show_base`, `show_walls`, `show_mech`) and dimensional parameters (`size`, `thick`, `rod_D`) are declared in the [project manifest](../project.json) and exposed for the Unit and Assembly modes.
...
This script generates a **single interlocking cube** by combining two half-cubes.
...
Bottom and top can be exported as separate STL files using `render_mode` 1 and 2.
...
These parameters are declared in the [project manifest](../project.json) and exposed for the Grid mode.
...
Each SCAD file uses a `render_mode` integer to select which part to export. The mapping is declared in the [project manifest](../project.json):
