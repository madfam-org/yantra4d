...
measured: 34 of the 500 cartridges live in submodules and **every** dual-engine cartridge
...
| :-- | --: | --: | --: | --: | --: |
| Cartridges declaring `project.engine: cadquery` | 474 | 500 | 94.8% | 300 / 326 = 92.0% | +2.8 pp |
...
| Cartridges declaring ≥1 constraint | 400 | 500 | 80.0% | 287 / 326 = 88.0% | -8.0 pp |
| Bilingual constraints per cartridge | 759 | 500 | 1.52 | 555 / 326 = 1.70 | -0.18 |
| Cartridges carrying `hyperobject.material_awareness` | 484 | 500 | 96.8% | 310 / 326 = 95.1% | +1.7 pp |
| …of which declare `tolerance_by_material` | 463 | 500 | 92.6% | 294 / 326 = 90.2% | +2.4 pp |
...
| Studio strings translated across de/fr/pt/zh | 1040 | 1460 | 71.2% | ~992 / 1308 = 75.8% | -4.6 pp |
...
**Quadrilingual coverage.** de/fr/pt/zh carry 944 of 1,364 strings, 69.2% — down 6.6 pp
from the audit's ~75.8%. Nothing was un-translated. The studio's key set grew from 327 to
341, the four locales absorbed the new keys as `[UNTRANSLATED]` markers, and the per-locale
marker count went from ~79 to 105. Key parity is intact (six files, 341 keys each), which
is precisely why a parity gate cannot see this regression, and why the mechanism audit #6
described is unchanged: `resolveTranslation` in `LanguageProvider.tsx:26-28` returns the
stored string whenever it is truthy, and `"[UNTRANSLATED] …"` is truthy, so the marker
reaches the user instead of the English fallback. The CI half of #6 is also still open —
`scripts/qa/i18n_audit.py` appears in no workflow on `main`. Every key added since the
audit has widened this gap by construction, and will keep doing so until either the
...
**The public discovery surface.** 326 of 500 cartridges are reachable from the landing
gallery — the static `apps/landing/src/data/projects.ts` array audit #8 flagged. It is
machine-generated by `scripts/dev/generate-landing-projects.mjs`, but it has not been
regenerated since: only 2 of the 176 cartridges added after the audit appear in it, and 2
of its 328 entries are cartridges the Commons catalog deliberately does not publish. 174
cartridges — over a third of the commons — are invisible to anyone not already inside
Studio. This is the largest single value gap the recomputation found, and the audit ranked
it eighth.
...
audit had one; the movement is mostly a consequence of what the numbers now say.

| # | Gap | What the recomputation measures | Why it ranks here |
| :-- | :-- | :-- | :-- |
| 1 | **Regenerate the landing gallery and migrate it to `/api/catalog/search`** [#8] | 326 / 500 reachable; 2 of 176 post-audit cartridges present; 2 entries the catalog excludes | A third of the commons is unfindable by the public. Regeneration is a script run today; the migration removes the drift permanently |
...
| 4 | **Close the quadrilingual gap and gate i18n in CI** [#6] | 69.2%, −6.6 pp; 105 markers per locale; `i18n_audit.py` in no workflow on `main` | The only metric that regresses automatically with every merge; the gate is cheaper than the translations and stops the bleeding first |
