...
interface UploadStepProps {
  ...
  files: File[]
  handleFileDrop: (e: React.DragEvent | React.ChangeEvent<HTMLInputElement>) => void
  handleAnalyze: () => void
  loading: boolean
  t: (key: string) => string
}

export default function UploadStep({ slug, setSlug, files, handleFileDrop, handleAnalyze, loading, t }: UploadStepProps) {
  return (
    <div className="flex flex-col gap-4">
      ...
      <div>
        ...
        <input
          type="text"
          value={slug}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSlug(e.target.value.replace(/[^a-z0-9-_]/gi, ""))}
      ...
      <div
        onDrop={handleFileDrop as React.DragEventHandler}
        onDragOver={(e: React.DragEvent) => e.preventDefault()}
        className="border-2 border-dashed border-border rounded-lg p-4 sm:p-8 text-center cursor-pointer hover:border-primary/50 transition-colors"
      >
        ...
        <input
          ...
          multiple
          accept=".scad"
          onChange={handleFileDrop as React.ChangeEventHandler<HTMLInputElement>}
          className="hidden"
          id="scad-upload"
        />
