...
import { useAssemblyEditor } from '../../hooks/editor/useAssemblyEditor'
...
interface AssemblyEditorPanelProps {
  onStepChange: (step: Record<string, unknown>) => void
  onClose: () => void
  viewerRef: React.RefObject<unknown>
  projectSlug: string
}

export default function AssemblyEditorPanel({ onStepChange, onClose, viewerRef, projectSlug }: AssemblyEditorPanelProps) {
  ...
  const allParts = manifest?.parts?.map((p: Record<string, unknown>) => p.id as string) || []
      ...
      {selectedStep && (
        <>
          ...
          <PartVisibilityPicker
            ...
            onChange={(visible: string[], highlight: string[]) => {
