...
**Remediation**: Fixed. Added `AVAILABLE_GLBS` allowlist in `ProjectCarousel3D.tsx` with the 5 slugs that have pre-rendered GLBs. Projects not in the set render `WireframeFallback` directly, skipping the GLB fetch entirely.
...
6. **NEW-3 (GLB 404 console noise)**: ✅ FIXED — Added `AVAILABLE_GLBS` allowlist to skip fetch for missing models.
