...
| Mobile layout | Studio mobile (<768px): header overflow DropdownMenu, editor as bottom Sheet (landscape capped at 60vh), AI dismiss backdrop. Landing mobile: carousel FOV reactive via matchMedia listener, ContactShadows skipped, dpr capped 1.5. Landing header Escape key closes mobile menu. Hero animations prefixed with `motion-safe:`. Admin sidebar slides in as mobile overlay with backdrop |
| Safe areas | Studio + Landing use `viewport-fit=cover` and `env(safe-area-inset-*)` CSS utilities (`pb-safe`, `pt-safe`, `px-safe`) for notched devices. Applied to: studio PrintEstimateOverlay, landing header, mobile menu, footer. Tailwind `xs: 360px` and `landscape` custom screen variants in both configs |
...
| Admin responsive | AdminShell has collapsible sidebar: fixed overlay on mobile (<md) with backdrop, relative layout on desktop. Hamburger toggle in mobile header. ProjectList table has `overflow-x-auto` with `min-w-[500px]` |
