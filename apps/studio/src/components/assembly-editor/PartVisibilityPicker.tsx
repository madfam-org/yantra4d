...
interface PartVisibilityPickerProps {
  allParts: string[]
  visibleParts: string[]
  highlightParts: string[]
  onChange: (visibleParts: string[], highlightParts: string[]) => void
}

export default function PartVisibilityPicker({ allParts, visibleParts, highlightParts, onChange }: PartVisibilityPickerProps) {
  const toggleVisible = (partId: string) => {
  ...
  const toggleHighlight = (partId: string) => {
