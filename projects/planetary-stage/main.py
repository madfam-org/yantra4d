...
def build_ring():
    ...
    ro = rp_ring + module                          # for the internal tooth flank sampling
    rr = max(r_tip_inner, rb)
    tooth = _one_tooth_ext(teeth_ring, rb, ro, rr, flank_pts)
    # Scale the tooth radially inward slightly by backlash (shrink toward centre).
    if ring_backlash > 0.0:
        shrink = 1.0 - (ring_backlash / max(rp_ring, 1.0))
        tooth = [(x * shrink, y * shrink) for (x, y) in tooth]
    ...
    clip = cq.Workplane("XY").circle(r_root_outer + 0.01).extrude(gear_thick)
