...
// --- CDG Math Functions ---
// Calculates the total physical width required for a slide slot, including tolerances
function slot_width(slide_thick, tol_z) = slide_thick + tol_z + 0.2;
// Calculates the pitch (distance between slots centroids)
function pitch(slot_w, rib_w) = slot_w + rib_w;

// --- Core Modules ---

// Generates a vertical rib meant to hold a slide in place
// Built using a polygon extrusion to create a tapered fin with an optional chamfer at the top
module aocl_retention_rib(height, depth, root_w, tip_w, chamfer_h) {
  ...
  rotate([-90, 0, 0]) translate([0, -height, 0]) {
      // Extrude the main part of the rib
      linear_extrude(height=depth)
        polygon([[0, 0], [root_w, 0], [root_w - _off, _body], [_off, _body]]);

      // Extrude the tapered/chamfered top guide
      if (_ch > 0) {
        _w = tip_w;
        translate([_off, 0, 0])
          linear_extrude(height=depth)
...
// Generates an anchorable recess block that will later be subtracted to make a labeling surface
module aocl_label_recess(w, h, d = 0.4) {
  cuboid([w, h, d], anchor=BOTTOM);
}

// Generates a snap-fit cantilever arm used mainly on the lid
// Provides a flexible stem and a hook at the top for latching onto a corresponding catch
