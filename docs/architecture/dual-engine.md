...
- **Verification**: The parity check that runs in CI is `tests/scripts/geometric_regression.py --tolerance 0.05`, in the `test-geometric-parity` job. `scripts/qa/verify_parity.py` renders both engines and compares the resulting 3D volumes for a single cartridge, but **no workflow invokes it** — it is a local tool.
- **Candidate selection**: a mode is compared only when both kernels really exist for it — a `scad_file` that ends in `.scad` and is present, plus a CadQuery source ending in `.py` (declared, or inferred from the `.scad` sibling) that is present. This mirrors `scripts/qa/generate_commons_catalog.py::_engine_support` and must keep mirroring it: CadQuery-only cartridges declare `scad_file` pointing at their own `.py` as a placeholder, and handing that to OpenSCAD asks it to parse Python. Placeholder modes are reported as skips, counted separately from failures.
- **Why it is still not a CI lane**: measured on 2026-09-02 over the full commons (then 35 public cartridge submodules plus `libs/`; since RFC 0038 P2 the commons is ONE submodule at `projects/`, and the figures below have not been retaken against it — nor against the 500-cartridge, 20-dual-engine shape the catalog reports today, which is itself past the 495/19 shape ADR-021 left when it removed `stemfie` and took `gridfinity` CadQuery-only, emptying `VERIFIED_PARITY_PROJECTS`), the audit reports **18 of 28 comparable pairs passing** — 10 pairs still do not. That is the figure *after* #115 pinned ten parity-fixed cartridges at their merged mains and `implicit-lattice-hyperobject` stopped declaring an OpenSCAD kernel it never implemented: 29 candidate pairs → 28, 10 passing → 18. The run before those pins took **5m38s** for 10 of 29 and failed on 17 geometric divergences, one CadQuery sandbox rejection and one OpenSCAD render failure; it also recorded 1417 modes with no comparable pair at all (1397 CadQuery-only placeholders, 20 OpenSCAD-only). Neither the per-cause split of the 10 that remain, nor the wall clock, nor that no-pair census has been retaken since the pins. So the Parity Guarantee below describes an intent the commons does not currently meet; wiring the tool into CI would fail the build on day one. Closing the remaining 10 gaps is the prerequisite, not a change to the tool.
