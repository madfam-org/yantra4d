...
interface AiChatPanelProps {
  mode: 'configurator' | 'code-editor'
  projectSlug: string
  manifest?: Record<string, unknown>
  params?: Record<string, unknown>
  setParams?: (updater: (prev: Record<string, unknown>) => Record<string, unknown>) => void
  onApplyEdits?: (edits: CodeEdit[]) => void
  fileContents?: Record<string, string>
}

export default function AiChatPanel({ mode, projectSlug, params, setParams, onApplyEdits, fileContents }: AiChatPanelProps) {
  ...
  } = useAiChat({ projectSlug, mode, params: params || {}, setParams } as Parameters<typeof useAiChat>[0])
  ...
  const handleApplyAll = () => {
    if (onApplyEdits && pendingEdits.length > 0) {
      onApplyEdits(pendingEdits as CodeEdit[])
      setPendingEdits([])
    }
  }
              ...
              <div key={i} className="flex flex-wrap gap-1">
                {Object.entries(msg.changes || {}).map(([k, v]) => (
                  <span key={k} className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] rounded bg-primary/15 text-primary">
                    <Check className="h-2.5 w-2.5" /> {k} → {String(v)}
                  </span>
                ))}
              </div>
            )
