...
const STEPS = ['url', 'review', 'confirm'] as const

interface ValidationResult {
  scad_files: Array<{ path: string; size: number }>
...
export default function GitHubImportWizard({ onClose, onImported }: GitHubImportWizardProps) {
            ...
            <>
              <div>
                ...
                <input
                  ...
                  type="text"
                  value={slug}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-_]/g, ''))}
                  className="w-full px-3 py-2 text-sm rounded-md border border-border bg-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                />
              </div>
              <div>
