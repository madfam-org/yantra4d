...
// Available GLBs loaded from manifest generated by scripts/prerender-carousel.sh
function useAvailableModels() {
    const [models, setModels] = useState<Set<string>>(new Set());
    useEffect(() => {
        fetch('/models/manifest.json')
            .then(r => r.ok ? r.json() : { models: [] })
            .then(data => setModels(new Set(data.models.map((m: { slug: string }) => m.slug))))
            .catch(() => setModels(new Set()));
    }, []);
    return models;
}

function LiveModel({ project, availableModels }: { project: any; availableModels: Set<string> }) {
    if (!availableModels.has(project.slug)) {
...
function CarouselItem({ project, index, total, radius, availableModels }: { project: any, index: number, total: number, radius: number, availableModels: Set<string> }) {
    ...
    return (
        <group ref={groupRef as any} position={[x, 0, z]} rotation={[0, angle, 0]}>
            <LiveModel project={project} availableModels={availableModels} />
        </group>
    );
}

function CarouselTrack({ projects, onActiveChange, availableModels }: { projects: any[], onActiveChange: (idx: number) => void, availableModels: Set<string> }) {
                ...
                <Suspense fallback={null}>
                    {carouselProjects.length > 0 && (
                        <ScrollControls
                            ...
                            distance={1}
                        >
                            <CarouselTrack projects={carouselProjects} onActiveChange={setActiveIndex} availableModels={availableModels} />
                        </ScrollControls>
                    )}
