import React from 'react';
import { CATEGORIES } from '../data/projects';
import { STUDIO_URL } from '../lib/env';
import type { Translations } from '../lib/i18n';
import type { GalleryItem } from '../lib/gallery';

type GridProject = Pick<GalleryItem, 'slug' | 'name' | 'category'> & Partial<GalleryItem> & { descriptionEs?: string };

type Props = {
  lang?: string;
  t?: Translations;
  projects: GridProject[];
  activeCategory: string;
  setActiveCategory: (val: string) => void;
  categoryLabels?: Record<string, string>;
  domainLabels?: Record<string, string>;
  openLabel?: string;
  hyperobjectLabel?: string;
  /** The category tablist; the island hides it while its own toolbar is showing. */
  showTabs?: boolean;
};

const DOMAIN_LABELS: Record<string, { en: string; es: string }> = {
  household: { en: 'Household', es: 'Hogar' },
  industrial: { en: 'Industrial', es: 'Industrial' },
  medical: { en: 'Medical', es: 'Médico' },
  commercial: { en: 'Commercial', es: 'Comercial' },
  hybrid: { en: 'Hybrid', es: 'Híbrido' },
  culture: { en: 'Culture', es: 'Cultura' },
};

/**
 * The client-rendered grid: filtered results and pages after the first. The
 * first page is server-rendered by `ProjectCard.astro` — same card, kept in step.
 */
export default function ProjectGalleryGrid({
  lang = 'es',
  t,
  projects,
  activeCategory,
  setActiveCategory,
  categoryLabels: categoryLabelsProp,
  domainLabels: domainLabelsProp,
  openLabel: openLabelProp,
  hyperobjectLabel: hyperobjectLabelProp,
  showTabs = true,
}: Props) {
  const isEs = lang === 'es';

  const categoryLabels: Record<string, string> = categoryLabelsProp ?? {
    all: t?.gallery.categories.all ?? 'Todos',
    commons: (t as any)?.gallery?.categories?.commons ?? '🔷 Commons',
    storage: t?.gallery.categories.storage ?? 'Almacenamiento',
    mechanical: t?.gallery.categories.mechanical ?? 'Mecánico',
    art: t?.gallery.categories.art ?? 'Arte y Generativo',
    tabletop: t?.gallery.categories.tabletop ?? 'Mesa de Juego',
    education: t?.gallery.categories.education ?? 'Educación',
    electronics: t?.gallery.categories.electronics ?? 'Electrónica',
  };
  const domainLabel = (domain: string) =>
    domainLabelsProp?.[domain] ?? (isEs ? DOMAIN_LABELS[domain]?.es : DOMAIN_LABELS[domain]?.en) ?? domain;
  const openLabel = openLabelProp ?? t?.gallery.openInStudio ?? 'Abrir en Studio →';
  const hyperobjectLabel = hyperobjectLabelProp ?? (isEs ? 'Hiperobjeto' : 'Hyperobject');

  return (
    <div>
      {showTabs && (
        <div className="flex overflow-x-auto snap-x gap-2 justify-start sm:justify-center sm:flex-wrap mb-8 pb-1" role="tablist" aria-label="Project categories">
          {CATEGORIES.map((cat) => (
            <button
              key={cat}
              role="tab"
              aria-selected={activeCategory === cat}
              onClick={() => setActiveCategory(cat)}
              className={`snap-start shrink-0 px-4 py-2.5 min-h-[44px] text-sm rounded-full border transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ${
                activeCategory === cat
                  ? 'bg-primary text-primary-foreground border-primary'
                  : 'bg-background text-muted-foreground border-border hover:text-foreground'
              }`}
            >
              {categoryLabels[cat] ?? cat}
            </button>
          ))}
        </div>
      )}

      <div className="grid gap-4 phone-landscape:grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {projects.map((project) => {
          const description = isEs ? project.descriptionEs ?? project.description : project.description;
          return (
            <a
              key={project.slug}
              href={`${STUDIO_URL}/project/${project.slug}`}
              target="_blank"
              rel="noopener noreferrer"
              data-testid="commons-card"
              data-slug={project.slug}
              className={`group rounded-xl border overflow-hidden transition-shadow hover:shadow-lg focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ${
                project.isHyperobject
                  ? 'border-blue-300 dark:border-blue-700 bg-gradient-to-b from-blue-50/50 to-card dark:from-blue-950/20 dark:to-card'
                  : 'border-border bg-card'
              }`}
            >
              <div className="aspect-video bg-secondary/30 overflow-hidden relative">
                <img
                  src={project.thumbnail}
                  alt={project.name}
                  className="w-full h-full object-cover transition-transform group-hover:scale-105"
                  loading="lazy"
                  decoding="async"
                  width="640"
                  height="360"
                />
                {project.isHyperobject && (
                  <span className="absolute top-2 right-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-xs font-bold uppercase tracking-wider px-2 py-0.5 rounded-full shadow-sm">
                    🔷 {hyperobjectLabel}
                  </span>
                )}
              </div>
              <div className="p-4">
                <div className="flex items-center justify-between mb-1 gap-1">
                  <h3 className="font-semibold text-sm truncate min-w-0">{project.name}</h3>
                  <div className="flex gap-1 shrink-0">
                    {project.isHyperobject && project.domain && (
                      <span className="text-xs text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/40 rounded-full px-1.5 py-0.5 font-medium">
                        {domainLabel(project.domain)}
                      </span>
                    )}
                    <span className="text-xs text-muted-foreground bg-secondary/50 rounded-full px-2 py-0.5">
                      {categoryLabels[project.category] ?? project.category}
                    </span>
                  </div>
                </div>
                {description && <p className="text-xs text-muted-foreground mb-2 line-clamp-2">{description}</p>}
                {/* 12px call-to-action: text-primary measures 3.83:1 on the
                    card surface, under the 4.5:1 small-text floor. */}
                <span className="text-xs text-primary-readable group-hover:underline">{openLabel}</span>
              </div>
            </a>
          );
        })}
      </div>
    </div>
  );
}
