...
function ActionDock({ compareMode, onToggleCompare }: ActionDockProps) {
  ...
  return (
    <div className="p-4 bg-background/85 backdrop-blur-xl border-t border-border flex flex-col gap-2 z-20 shadow-[0_-4px_24px_rgba(0,0,0,0.05)] dark:shadow-[0_-4px_24px_rgba(0,0,0,0.2)] shrink-0">
      ...
      <Button
        ...
        disabled={loading || constraintErrors as unknown as boolean}
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        {loading ? (
          ...
          <Button
            ...
            disabled={constraintErrors as unknown as boolean}
...
function ModeTabs({ className }: ModeTabsProps) {
      ...
      >
        {manifest.modes.map((m: Record<string, unknown>) => (
          ...
          >
            {m.svg ? (
              <>
                <span className="flex items-center justify-center opacity-70" dangerouslySetInnerHTML={{ __html: m.svg as string }} />
...
function SidebarContent({ compareMode, onToggleCompare }: SidebarContentProps) {
  const {
    manifest, mode,
  ...
  const hasAssemblySteps = (manifest as Record<string, unknown>)?.assembly_steps != null && ((manifest as Record<string, unknown>).assembly_steps as unknown[])?.length > 0
  const currentModeParts = manifest.modes?.find((m: Record<string, unknown>) => m.id === mode)?.parts || []
  const showAssemblyGuide = hasAssemblySteps && ((manifest as Record<string, unknown>).assembly_steps as Array<Record<string, unknown>>).some(s =>
    ((s.visible_parts || []) as string[]).some(p => (currentModeParts as string[]).includes(p))
  )
