...
# QA lanes (repo root, stdlib only unless noted). All need the 35 public
# `projects/*` submodules: `git submodule update --init projects/`
python3 scripts/qa/validate_manifests.py            # every manifest schema-validates; an EMPTY submodule is a failure, not a skip (blocking in CI: manifest-validation)
